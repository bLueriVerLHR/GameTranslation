...
## 7. v2 单写者流程 + 烘焙后验收（2026-09，MZ 大插件集）

### 7.1 提取器与 keys.jsonl 的「不同一版本」陷阱
...
### 7.4 烘焙后残留扫描的「按设计跳过」必须逐条到引擎源码核实
...
### 7.6 blob 重建后写回构建（v1 合并路的 v2 等价物）
