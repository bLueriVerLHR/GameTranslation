...
- **启动 agent 前必须对照烘焙字典查术语**（第一次词表猜了角色名一个
  ...
  找每个角色名。
- **姿势/动画名消歧：** 对 `姿势类(左スパ)` 类标签，作者意图在事件正文
  （跑哪些图）与同族其他标签（表情类 = 表情）。别从无上下文假名
...
- **题材术语统一（owner 定案，本地 adult 词表已记录）：** 主动题材术语
  类词 → **题材术语**；被动被题材术语类词 → **题材术语**（或直接用题材术语）。补翻 agent
...
- **修复遍历：** 残留假名行里除豁免外全是题材术语串 — 机械替换两个
  术语（题材术语/题材术语）后全部清零。double-backslash 告警要逐条看：插件参数
...
- **内嵌 `cn.rar` 不是汉化包：** 另一款包内嵌 `cn.rar`，名字像汉化包，
  实际是推广包（内含"卸载类"exe 推广，密码同 发布渠道，见本地
...
- **广告文件：** 根目录推广文本（广告/推广渠道类 `.txt`）、推广 `.url` 均在网页根
