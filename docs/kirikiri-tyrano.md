...
不要把页码、临时选择或播放位置放进去。游戏专属变量表保存在本地工作区或
跨游戏通用词表目录，不提交到公开仓库。
...
这一节把「什么算做完」写清楚。**下面每一层都不能代替上一层验证**，尤其是
桌面浏览器结果**不能**当作 Android 实机验收。
...
| 用户试玩 | 逐项确认反馈，记录仍需调整的取舍 | 自动化回归 |

抽查时**随机抽样不同场景**，不要反复测同一个已知正确的场景；定点回归用来
确认修复，随机抽样用来发现破坏，二者不能互相替代。

### 7.1 快进与调试开关

- 调试快进（`--fast-skip`）与正常游玩**分开**：正常模式保留原作节奏。
- 允许缩短视觉等待与即时出字；**保留**变量赋值、条件判断、跳转与必需初始
  化；遇到选择或明确停止点立即停住。
- 已知行为：快进不越过 `[p]`（改页点击等待）。交付构建（无 `--fast-skip`）
  对玩家无影响；若希望“开快进就一直跑”，在**调试/测试构建**里带该选项。
- 测量加速时按耗时来源分开统计（文字 / 定时等待 / 转场 / 媒体），**不要**
  把所有等待一律删掉。

### 7.2 手机控制入口

先盘点原作每个按钮的**实际用途**，建立「原作行为 → 兼容实现 → 当前状态」
表，再实现。不能只把原作按钮接到 Tyrano 菜单后就算完成。
**未支持的按钮应明确标示或隐藏，不能呈现为可用但点击无效果。**

存读档必须覆盖兼容层自己的状态（图层、页、当前语音/动画策略、变量），
不能只接引擎的存读档界面就宣告完成。

### 7.3 降级必须记录

按真实用途区分视频、逐帧动画、立绘移动/缩放与转场效果，逐项确认时序、
透明度、层级、循环与停止条件。**不支持的效果记进能力表，不静默变成空操作**
（空操作当成功是本项目反复踩过的坑）。

当前已知降级（随修复更新）：旧插件动画以**代表帧回退**呈现（避免进入动画
鉴赏后只显示黑屏），视频型内容复用已转换的 WebM 播放路径；取得可还原的
帧序列或时序数据后再恢复完整循环。

### 7.4 工期与进度

**不承诺整体工期**：界面问题需先复现，动画与鉴赏范围需先盘点。每个阶段
完成后记录真实耗时、剩余缺口与下一份可试玩产物。历史进度记录（含各轮
试玩反馈与修复轨迹）归档在 `docs/archive/kirikiri-mobile-progress.md`。
