...
- **自动适配** `window.__kag3_fit_message(el)`：先换成**单位化行高 `1.25`**
  （对齐 KAG3，且随字号等比缩放 ⇒ 收敛单调），仍超出则按 4% 步长等比缩字号，
  下限 0.72；每次都从基线样式重新测量，短消息自动恢复原字号。
  由 **150ms 去抖的 MutationObserver** 触发——打字机每帧重写文本，逐帧测布局
  会强制每帧回流（§3.1 的 TMP 热路径教训同源）。
...
**测不到真几何的边界**：把 `.layer` 强制 `display:block` 只能量到 Tyrano 的
**默认**几何（918×598），游戏自己的 `[position layer=messageN …]` 只在场景
流程里生效（且开场流程未走完时消息层不可见）。因此本节用“同几何合成窗口”
做机制验证，真实画面验证留给 owner 试玩。
