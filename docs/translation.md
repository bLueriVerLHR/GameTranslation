...
> 下文描述的是 v1（切块 → 并行 subagent → 逐行 ja/zh 对齐 → 合并 → 烘焙）。它的教训仍然有效
> （失败模式目录、QC 清单、术语一致性审计都在下文），但**流程与工具已被上面的 v2 取代**。

# RPG Maker 翻译指南（统一版）

本工具库唯一的翻译指南。它把经典全量翻译流程和残留补翻流程合并成
**一套工作流、一套参数**（2026-08 长程验证）：**默认每轮 10 个并行
subagent（受 harness 并发上限约束）** 和 **按 90KB 上下文预算自动选档
的大块**（标定值约 11,000 字符/块）。所有踩过的失败模式、QC 关卡、修复
手法都记录在本文档——工具已加固、prompt 契约已定稿。下文的具体数字
（并行度、字符数、成功率）都是**某一套条件下的历史标定值**，换了模型、
harness 或游戏体量就要重新测（生成器会打印实际的块数与 context 大小）。
...
### 第 3.5 步 — 并行度（默认每轮 10 个，受 harness 上限约束）

**默认每轮 10 个并行 subagent** — 这是历史标定值（9-11 都验证稳定），
**不是上限也不是硬保证**：harness 并发上限低于 10 时取小者，其余策略不变。
分批追加契约下首轮成功率实测 ~100%（三轮 0 失败），也出现过 ~90% ——
差异**取决于 prompt，不是数量**。策略：

- **每轮 10 个 agent**（默认，取 harness 上限与 10 的较小值）；轮次数量由
  实际块数 ÷ 并行度决定，不固定。
