...
> 2026-09 定案并在真实作品上跑通（24047 键全量翻译 → 四门禁全绿 → 烘焙 → 试玩 → 交付）。
> v2 取代 v1 的「切块 → 每块一个 subagent → 逐行对齐 → 合并 → 烘焙」管线；
> v1 的描述保留在本文档下半部分，**仅作历史参考**（对应工具待退役，见 TODO）。
...
- `qc_build_kana.py` 的插件部分除假名清单外，还会输出 **kana-free CJK 待复核清单**
  （`REVIEW` 行，只报不判失败）——逐条对照 `docs/table/<Game>/decisions.md` 的
...
按本地策略表 `docs/table/font_rollback.md`：MV 改 `fonts/gamefont.css` 单一 face → 简中字体（原字体保留）；
...
> （失败模式目录、QC 清单、术语一致性审计都在下文），但**流程与工具已被上面的 v2 取代**。
...
> 具体成人词汇的中日对照与判定表属于游戏专属数据，只放本地
> `docs/table/`（gitignored），**绝不写进本仓库文档**（公开仓库卫生规则）。
...
| 中文显示方块 | 烘焙器自动应用**标准字体策略**（`--cjk-font` 中文字体 + `--jp-font` 日文 fallback；解析顺序：环境变量 → `docs/table/local_font_path.txt` 首/二行 → 自动发现 `docs/table/fonts/`；见 `docs/workflow.md`）。仍不行就装 CJK 字体（微软雅黑 / Noto Sans CJK） |
