...
RUNTIME_SHIM_IIFE = "\n".join([
    ...
    "          if (!el.__kag3_base) {",
    "            el.__kag3_base = { fs: parseFloat(getComputedStyle(el).fontSize) || 28 };",
    "          }",
    "          var base = el.__kag3_base;",
    "          // start from the base style so a short message after a long one",
    "          // is not left shrunk, and use a unitless KAG3-like line height:",
    "          // it tracks the font size, which keeps the fit monotone.",
    "          el.style.fontSize = '';",
    ...
    "          while (k > 0.72 && !fits()) {",
    "            k -= 0.04;",
    "            el.style.fontSize = (base.fs * k) + 'px';",
    "          }",
    "          return true;",
