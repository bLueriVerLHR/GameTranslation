...
# re-exported: tools and the KAG passes import these two names from here, and
# moving the implementation to core must not break their call sites.
...
def load_ks(path: str) -> tuple[str, str]:
    ...
    Delegates to `rpgmaker.textencoding.load_text_file`: the heuristic is
    shared with TyranoScript, and one copy is what keeps the two engines
