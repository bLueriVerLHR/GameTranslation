#!/usr/bin/env python3
"""japanese.py - shared Japanese-text detection regexes (core).
...
This module is core, not a tool: engines import it directly, so it must be
importable without ``tools/`` on ``sys.path`` (``kirikiri/ks_extract.py`` and
``tyrano/tyrano_extract.py`` use KANA for residual checks).  ``tools/`` keeps
a deprecated re-export for one compatibility cycle.
"""
