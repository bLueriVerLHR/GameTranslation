...
This is core, not a tool: ``rpgmaker/plugincompat.py`` (core) parses
``js/plugins.js`` through it, so it must be importable without ``tools/`` on
``sys.path``.  ``tools/plugins_io.py`` keeps a deprecated re-export for one
compatibility cycle.
...
def _scan_string(text, i):
    ...
    while i < len(text):
        ...
        if c == '"':
            try:
                return json.loads('"{}"'.format("".join(out))), i + 1
            except ValueError as exc:
                raise ValueError("bad escape sequence in string") from exc
        out.append(c)
        i += 1
...
def _parse_object(text, i):
    """Parse `{...}` starting at the `{`; returns `(obj, next_index)`."""
    i += 1
    obj = {}
    while True:
        i = _skip_ws(text, i)
...
def _parse_array(text, i):
    """Parse `[...]` starting at the `[`; returns `(arr, next_index)`."""
    i += 1
    arr = []
    while True:
        i = _skip_ws(text, i)
...
def _parse_scalar(text, i):
    """Parse a number/`true`/`false`/`null`; returns `(value, next_index)`."""
...
def _parse_value(text, i):
    """Parse one JS value at `i`; returns `(value, index past it)`.

    Dispatch only; the container and scalar parsers live above so each error
    message stays next to the branch that raises it.
    """
    i = _skip_ws(text, i)
    if i >= len(text):
    ...
    for p in plugins:
        ...
        for key, val in items:
            ...
            if not ja_re.search(val):
                continue
            yield f"js/plugins.js / {name} / {key}", val
...
def _json_string(text):
    """Decode one JS string literal, or `_NOT_A_STRING` when it is not one.

    The `try` sits in its own function so the caller's loop is a plain scan;
    the sentinel is needed because a valid literal can decode to None.
    """
    try:
        return json.loads(text)
    except ValueError:
        return _NOT_A_STRING


def iter_literals(text):
    """Fallback textual pass: yield every JS string literal in order."""
    for m in JS_STR.finditer(text):
        value = _json_string(m.group(0))
        if value is not _NOT_A_STRING:
            yield value
