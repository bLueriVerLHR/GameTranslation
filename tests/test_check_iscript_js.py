...
Parsing is in-process (`rpgmaker.jssyntax`, tree-sitter) instead of
`node --check`, so these tests need no Node.js install.
"""
import json
import os
import sys
...
from rpgmaker import jssyntax  # noqa: E402
...
def test_invalid_javascript_is_reported_with_file_and_offset(tmp_path):
    ...
    assert p["offset"] > 0
    # tree-sitter reports either an unexpected region or the token it could
    # not find; both carry a 1-based line:column
    assert p["error"].startswith("line ")
    assert ("unexpected" in p["error"]) or ("missing" in p["error"])
...
def test_the_real_regression_pattern_is_caught(tmp_path):
    ...
    # the getter's closing brace is NOT followed by a comma, so the parser
    # reports an unexpected identifier there.
...
def test_comma_form_is_valid_js_so_it_cannot_be_caught_here(tmp_path):
    # Boundary of this tool: with a comma after the getter, a leaked
    # `setter(ma){...}` parses as an ES6 method shorthand, so it is NOT a
    # syntax error and any parser stays silent - even though the accessor
    # is missing.  The hard failure (no comma) is what this tool catches; the
    # silent variant must be caught by the converter's own tests
    # (tests/test_tjs2js.py asserts no raw `setter(` survives conversion).
    ...
    assert checked == 1
    assert problems == [], (
        "if this ever fails, the parser started rejecting the comma form and "
        "the tool got stronger - update the note in tools/check_iscript_js.py")
...
def test_missing_parser_is_reported_not_silently_passed(monkeypatch,
                                                       tmp_path):
    ...
    monkeypatch.setattr(jssyntax, "_get_parser", no_parser)
    assert cij.main([str(tmp_path)]) == 2


def test_no_node_dependency_remains():
    """The swap that removed Node.js from this check must not creep back."""
    import inspect

    code = inspect.getsource(cij)
    assert "proctools" not in code
    assert "find_node" not in code
